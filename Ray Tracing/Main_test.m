...
%% Calculating Power of the lens
d_lens = thickness_top_lens + thickness_bot_lens + t_film;
R_bot_c = -R_bot;
P = (n_PDMS-1)*((1/R_top) - (1/R_bot_c) + (((n_PDMS-1)*d_lens)/(n_PDMS*R_top*R_bot_c)));
f_avg = 1/P;
P_diopters = P*1E+06;

if(P_diopters < 0)
choice = questdlg('For the given lens dimensions, the power is negative and may lead to formation of virtual image which may not be displayed properly.', ...
        'Negative Power for the Lens', ...
        'Continue Anyway','Stop','Continue Anyway');
    % Handle response
    switch choice
...
for i = 1:n
    ...
    phi = (abs(atand(double(m_tangent))));
    
    
    if(alpha_left_up(i,1)>=0)
        theta_left = abs(alpha_left_up(i,1) + phi);
    ...
    alpha_right_up(i,1) = theta_right - phi;
...
for i = 1:n_end
    ...
    alpha_right_up_botlens(i,1) = alpha_left_up_botlens(i,1) - (theta_right-theta_left);
    ...
    alpha_right_down_botlens(i,1) = alpha_left_down_botlens(i,1) + (theta_right-theta_left);
